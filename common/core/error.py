...
*Abstract*
    ...
    * raise Error(err_type,err_message) 형식으로 에러를 발생시킵니다.
    * 로깅시 Error()로 에러인스턴스를 생성하여 전달합니다 eg. logger.error(Error(err_type,err_message))
    * output.py 모듈의 set_error 사용시 인수로 발생한 에러를 그대로 전달합니다

        >>> EXAMPLE

        def do_something():
            raise Error(ERROR_TYPES.RUPNTIME_ERROR,'test error')
...
class Error(Exception):
    ...
    def __init__(self,err_type,message=''):
        """
        example : Error(ERROR_TYPES.IMAGE_READ_ERROR,'failed to download the image')
