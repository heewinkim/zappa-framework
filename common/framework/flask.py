...
class Flask(Flask_):
    ...
    def _after_request(self,response):
        ...
        try:
            ...
            self._logger.warning(Error(ERROR_TYPES.RUNTIME_ERROR,'logging time elapse failed - _after_request in Flask'))
    ...
    def validate_keys(self,object:dict,keys:list,value_check=False):
        ...
        for key in keys:
            if not key in object:
                raise Error(ERROR_TYPES.PARAMETER_ERROR,'No {} data offered'.format(key))
            elif object.get(key) is None:
                if value_check:
                    raise Error(ERROR_TYPES.PARAMETER_ERROR, 'Invalid {}'.format(key))